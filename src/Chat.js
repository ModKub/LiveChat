...
const MessageBox = ({message, messages, sendMessage, setMessage, openBox, closeMessageBox}) => {
  ...
  return <div
    ...
    <div className='flex justify-between' style={{ padding: chatStarted ? '15px' : '30px' }}>
      ...
      <div onClick={closeMessageBox} className='rounded-full cursor-pointer self-start hover:bg-blue' style={{ padding: '5px' }}>
        <i className='fas fa-times' style={{ fontSize: '20px' }} />
      </div>
    </div>
    {chatStarted ?
      <div className='bg-white text-black border-b border-blue-dark' id='scroll' style={{ padding: '30px 30px 5px 30px', overflowY: 'auto', height: '410px' }}>
        {messages.map((el, i) => {
